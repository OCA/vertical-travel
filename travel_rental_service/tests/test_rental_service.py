...
class test_rental_service(TransactionCase):

    def setUp(self):
        super(test_rental_service, self).setUp()
        # Clean up registries
        ...
        self.rental_model = self.registry("travel.rental.service")
    ...
    def test_import_rental(self):
        self.rental_model.create(
            self.cr, self.uid, self.vals, context=self.context)
        rental_import_model = self.registry("travel.rental.service.import")
